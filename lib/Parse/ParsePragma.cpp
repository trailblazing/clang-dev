...
// cleni, for pragma
struct PragmaCprofProcedureHandler : public PragmaHandler {
  PragmaCprofProcedureHandler(Sema &S, Parser &p)
      : PragmaHandler("cprof_procedure"), Actions(S), parser(p) {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducerKind Introducer,
		    Token &FirstToken);

private:          // cleni, modified 02-20-2017
  Sema &Actions;  // cleni, modified 02-20-2017
  Parser &parser; // cleni, modified 02-20-2017
};
...
void Parser::initializePragmaHandlers() {
  ...
  // cleni, for pragma
  CprofHandler.reset(new PragmaCprofProcedureHandler(Actions, *this));
  PP.AddPragmaHandler(CprofHandler.get());
}
...
void Parser::resetPragmaHandlers() {
  ...
  // cleni, for pragma
  PP.RemovePragmaHandler(CprofHandler.get());
  CprofHandler.reset();
}
...
#define OPENCLEXT(nm)                                                          \
  else if (ename->isStr(#nm)) if (Supp.is_##nm##_supported_extension(CLVer))   \
      f.nm = state;                                                            \
  else if (Supp.is_##nm##_supported_core(CLVer))                               \
	  PP.Diag(NameLoc, diag::warn_pragma_extension_is_core)                \
      << ename;                                                                \
  else PP.Diag(NameLoc, diag::warn_pragma_unsupported_extension) << ename;
#include "clang/Basic/OpenCLExtensions.def"
  else {
...
void PragmaOpenCLExtensionHandler::HandlePragma(Preprocessor &PP,
  ...
  if (PP.getPPCallbacks())
    PP.getPPCallbacks()->PragmaOpenCLExtension(NameLoc, ename, StateLoc, state);
}
...
void PragmaMSPragma::HandlePragma(Preprocessor &PP,
				  PragmaIntroducerKind Introducer, Token &Tok) {
  Token EoF, AnnotTok;
  EoF.startToken();
  EoF.setKind(tok::eof);
  AnnotTok.startToken();
...
// cleni, for pragma
/// \brief Handle the \#pragma cprof procedure directive.
void PragmaCprofProcedureHandler::HandlePragma(Preprocessor &PP,
					       PragmaIntroducerKind Introducer,
					       Token &Tok) {
  StringRef funcName;
  ...
  funcName = Tok.getIdentifierInfo() ? Tok.getIdentifierInfo()->getName()
				     : StringRef();
  ...
  Actions.ActOnPragmaCprofProcedure(funcName);
}
