...
Sema::Sema(Preprocessor &pp, ASTContext &ctxt, ASTConsumer &consumer,
    ...
    : ExternalSource(nullptr), isMultiplexExternalSource(false),
      ...
      DictionaryWithObjectsMethod(nullptr), MSAsmLabelNameCounter(0),
      GlobalNewDeleteDeclared(false), TUKind(TUKind), NumSFINAEErrors(0),
      CachedFakeTopLevelModule(nullptr), AccessCheckingSFINAE(false),
...
#define OPENCLEXT(Ext)                                                         \
  if (Context.getTargetInfo()                                                  \
	  ...
	  .is_##Ext##_supported_core(getLangOpts().OpenCLVersion))             \
...
// cleni, for pragma
void Sema::ActOnPragmaCprofProcedure(StringRef funcName) {
  IsCprofProcedure = true;
}
// cleni, for pragma
void Sema::ActOnPendingCprofPragma(FunctionDecl *FD) {
  if (IsCprofProcedure) {
    FD->setIsCprofFunction(true);
    IsCprofProcedure = false;
  }
}
...
LambdaScopeInfo *Sema::getCurGenericLambda() {
  if (LambdaScopeInfo *LSI = getCurLambda()) {
    return (LSI->AutoTemplateParams.size() || LSI->GLTemplateParameterList)
	       ? LSI
	       : nullptr;
  }
  return nullptr;
